...
public class ClienteImplementazionePostgresDAO implements ClienteDAO {
    ...
    public List<Ordine> ottieniStoricoOrdini(Cliente cliente) {
        ...
        String query = "SELECT " +
                ...
                "FROM ordine " +
                "JOIN pagamento ON pagamento.id_ordine = ordine.id_ordine " +
        ...
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(query)) {
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    // 1. Creiamo l'oggetto Ordine base
                    model.commerce.Ordine ordine = new model.commerce.Ordine();
                    ordine.setId_ordine(resultSet.getInt("id_ordine"));
                    ordine.setData_ordine(resultSet.getDate("data_ordine").toLocalDate());

                    // Conversione pulita dell'Enum dello stato
                    String statoString = resultSet.getString("stato_ordine");
                    ordine.setStato(model.enums.StatoOrdine.valueOf(statoString.trim().toUpperCase()));

                    ordine.setTotale(resultSet.getBigDecimal("totale_ordine"));

                    // 2. Creiamo l'oggetto Pagamento associato
                    model.commerce.Pagamento pagamento = new model.commerce.Pagamento();
                    pagamento.setId_pagamento(resultSet.getInt("id_pagamento"));
                    pagamento.setImporto(resultSet.getBigDecimal("importo_pagamento"));
                    pagamento.setMetodo(resultSet.getBigDecimal("importo_pagamento") != null ? resultSet.getString("metodo_pagamento") : null);

                    // 3. Infiliamo il pagamento nell'ordine e aggiungiamo alla lista
                    ordine.setPagamento(pagamento);
                    storicoOrdini.add(ordine);
                }
            }
        } catch (SQLException e) {
            System.out.println("Errore nel recupero dello storico ordini semplificato-> " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public List<model.commerce.OrdineDettaglio> ottieniDettagliOrdine(int idOrdine) {
        List<model.commerce.OrdineDettaglio> listaDettagli = new ArrayList<>();

        // Query esplicita senza alias abbreviati: unisce il dettaglio dell'ordine al prodotto e alla sua categoria
        String query = "SELECT " +
        ...
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(query)) {
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    // 1. Estrazione dei dati per la Categoria del Prodotto
                    Categoria categoria = null;
                    int idCategoria = resultSet.getInt("id_categoria");
                    if (!resultSet.wasNull()) {
                        String nomeCategoria = resultSet.getString("cat_prodotto");
                        categoria = new Categoria(idCategoria, nomeCategoria);
                    }

                    // 2. Creazione dell'oggetto Prodotto completo con tutti e 5 i parametri del costruttore
                    ...
                    // 3. Creazione dell'oggetto OrdineDettaglio (Prodotto + Quantità acquistata)
                    int quantita = resultSet.getInt("quantita");
                    OrdineDettaglio dettaglio =new OrdineDettaglio(prodotto, quantita);

                    // 4. Aggiunta alla lista dei dettagli dell'ordine selezionato
                    listaDettagli.add(dettaglio);
                }
            }
        } catch (SQLException e) {
            System.out.println("Errore formale nel recupero dei dettagli del singolo ordine-> " + e.getMessage());
            e.printStackTrace();
        }
